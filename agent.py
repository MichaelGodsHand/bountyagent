    ...
    try:
        ...
        if bounty_result["success"]:
            ...
            try:
                ...
                ctx.logger.info(f"📝 Updated most recent company to: {msg.brand_name}")
...
async def handle_auto_generated_bounties(ctx: Context) -> AutoGeneratedBountiesResponse:
    ...
    try:
        ...
        if most_recent_company in generated_bounties:
            ...
            ctx.logger.info(f"📋 Returning bounties for most recent company: {most_recent_company}")
        else:
            ctx.logger.warning(f"⚠️ No bounties found for most recent company: {most_recent_company}")
